...
from datastructures.projects import ProjectForm
...
@route(
    request_method=app.get,
    path='/api/jaen/projects',
    status_code=status.HTTP_200_OK,
    payload_key=None,
    service_url=settings.JAEN_SERVICE_URL,
    authentication_required=True,
    post_processing_func=None,
    authentication_token_decoder='auth.decode_access_token',
    service_authorization_checker='auth.is_default_user',
    service_header_generator='auth.generate_request_header',
    response_model='datastructures.projects.ProjectResponse',
    response_list=True,
)
...
async def create_projects(project: ProjectForm, request: Request, response: Response):
    pass